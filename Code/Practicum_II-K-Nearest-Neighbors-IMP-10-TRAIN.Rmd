...
## Create KNN train and test data sets.
```{r BasketballKNNTrainTest}
set.seed(4567)
BasketballKNNTrainLabels <- as.factor(BasketballTrainIMP$RESULT)
BasketballKNNTestLabels <- as.factor(BasketballTestIMP$RESULT)
BasketballKNNTrain <- subset(BasketballTrainIMP, select = -c(RESULT))
BasketballKNNTest <- subset(BasketballTestIMP, select = -c(RESULT))
nrow(BasketballKNNTrain)
nrow(BasketballKNNTest)
head(BasketballKNNTrain, 2)
head(BasketballKNNTrainLabels, 2)
head(BasketballKNNTest, 2)
head(BasketballKNNTestLabels, 2)
str(BasketballKNNTrain)
str(BasketballKNNTrainLabels)
str(BasketballKNNTest)
str(BasketballKNNTestLabels)
```

## Run K Nearest Neighbors
```{r BasketballKNN}
system.time(BasketballKNN <- knn(train = BasketballKNNTrain, test = BasketballKNNTest, cl = BasketballKNNTrainLabels, k = 1))
```

```{r BasketballKNNPred}
BasketballKNN
table(BasketballKNN)
as.factor(BasketballKNNTestLabels)
CrossTable(x = BasketballKNNTestLabels, y = BasketballKNN, prop.chisq = FALSE)
```

```{r BasketballKNNCaretConfusionMatrix}
confusionMatrix(BasketballKNN, as.factor(BasketballKNNTestLabels))
```

## Choosing k as the square root of the number of training records
```{r BasketballKNN_01}
system.time(BasketballKNN <- knn(train = BasketballKNNTrain, test = BasketballKNNTest, cl = BasketballKNNTrainLabels, k = 157))
confusionMatrix(BasketballKNN, as.factor(BasketballKNNTestLabels))
```

## Choosing k as 5
```{r BasketballKNN_02}
system.time(BasketballKNN <- knn(train = BasketballKNNTrain, test = BasketballKNNTest, cl = BasketballKNNTrainLabels, k = 5))
confusionMatrix(BasketballKNN, as.factor(BasketballKNNTestLabels))
```

## Loop for different Accuracy
```{r BasketballKNNAccuracyLoopTest}
Accuracy <- 0
CurrentK <- 1

for (n in 1:200) {
  
  BasketballKNN <- knn(train = BasketballKNNTrain, test = BasketballKNNTest, cl = BasketballKNNTrainLabels, k = n)
  
  ConfusionMatrix <- confusionMatrix(BasketballKNN, as.factor(BasketballKNNTestLabels))
  
  ConfusionMatrixAccuracy <- ConfusionMatrix$overall['Accuracy']
  
  if (ConfusionMatrixAccuracy > Accuracy) {Accuracy <- ConfusionMatrixAccuracy 
  CurrentK <- n}
  
  cat(sprintf("K Value: %d, Accuracy: %f\n", n, Accuracy))
}
```

## Choosing k as 200
```{r BasketballKNN_02}
system.time(BasketballKNN <- knn(train = BasketballKNNTrain, test = BasketballKNNTest, cl = BasketballKNNTrainLabels, k = 200))
confusionMatrix(BasketballKNN, as.factor(BasketballKNNTestLabels))
```
