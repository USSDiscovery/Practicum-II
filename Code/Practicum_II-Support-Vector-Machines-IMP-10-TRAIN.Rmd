...
## SVM Radial
```{r BasketballSVMRadial}
set.seed(4567)
system.time(BasketballSVMTrainFit <- svm(RESULT~., 
                                         data = BasketballTrainIMP, 
                                         kernel = "radial", 
                                         cost = 1, 
                                         gamma = 1/ncol(BasketballTrainIMP)))
BasketballSVMTrainFit
```

## SVM Predict Radial
```{r BasketballSVMPredictRadial}
system.time(BasketballSVMPredict <- predict(BasketballSVMTrainFit, BasketballTestIMP[, !names(BasketballTestIMP) %in% c("RESULT")]))
BasketballSVMTestTable <- table(BasketballSVMPredict, BasketballTestIMP$RESULT)
BasketballSVMTestTable
classAgreement(BasketballSVMTestTable)
confusionMatrix(BasketballSVMTestTable)
```
